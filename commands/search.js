...
const {fetchJson,cmd, tlang, sleep, botpic, getBuffer, prefix, Config } = require('../lib')
...
cmd({
    ...
    async(Void, citel, text) => {
        ...
        let image = await fetchJson(`https://darkalphaxteam-api.cyclic.app/api/search/google-image?query=${text}&apikey=prabath`);
        const img = image.data[0];
        let google = require('google-it');
        google({ 'query': text}).then(res => {
                for (let g of res) {
            let no = 1;
            let msg= `*【🐲】ᴅʀᴀɢᴏɴ ᴍᴅ ɢᴏᴏɢʟᴇ sᴇᴀʀᴄʜ【🐉】* \n\n`;
                msg+= `*🔢 NO :* ${no++}`
                msg+= `_*🗳️ TITLE :*_ ${g.title}\n`;
                msg+= `_*📋 DESCRIPTION :*_ _${g.snippet}_`;
                msg+= `_*🖇️ URL :*_ ${g.link}\n\n──────────────\n\n`;
            }
